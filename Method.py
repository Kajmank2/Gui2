...
def Start():
    ...
    ListSensorneighQresult.append('#')
    ListSensorneighQresult.append("# iter  q  freqkD freqkC freqkDC freq_s_ON freq_s_DEAD")
                ...
                for x in converted_ListData:
                    ...
                    for y in flaten_list:
                        if (y[0] == '0' or y[0:2] == '5;' or y[0:2] == '8;'):  # SOLUCJA ZAPISAC CSV JAKO CIĄG STRINGÓW NIE OSOBNĄ LISTE
                            ...
                            ys = str(
                                circle(int(re.search(r'\d+', x[0:2]).group()), int(re.search(r'\d+', x[5:7]).group()),
                                       int(y[0:3]), int(y[4:]), int(Radius), 1))
                            if (ys[0] == '0'):
                                donothing()
                            else:
                                helper += 1
                            ...
                            ys = str(
                                circle(int(re.search(r'\d+', x[0:2]).group()), int(re.search(r'\d+', x[5:7]).group()),
                                       int(y[0:2]), int(y[3:]),int(Radius), 1))
                            if (ys[0] == '0'):
                                donothing()
                            else:
                                helper += 1
    ...
    #battery state plot
    def PlotBatter():
        plt.plot(BatterY_STATE_SUM)
        plt.xlabel("iter")
        plt.ylabel("Battery")
        plt.show()
        plt.savefig('avarage_Battery.png')
    PlotBatter()
    #FREQ PLOT
    def freqOnOff():
        plt.plot(FreqOff,'r')
        plt.plot(FreqOn,'g')
        plt.xlabel("iter")
        plt.ylabel("freq_s_ON,freq_s_OFF.png")
        plt.show()
        plt.savefig('freq_s_ON,freq_s_OFF.png')
    freqOnOff()
...
def Startstandard():
    ...
    def MainIter():
        ...
        NewState = []
        coverageQ=''
         #LIST Which contain q matplotlib
        StateListNeigh = []
        SensorHelper = []  # HELPER LIST
        POIVALUE = 441
        IdPOICOV = []
        ...
        for i in range(100):
            ...
            def CalcALLq():  # WRITE COVERAGE METHOD
                ...
                ListSensorneighQresult.append("    " + str(int(i)) + "  " +
                                              str(round(coverageQ, 2)) + "  " + kd + "  " + kc + "  " + kdc + "  " +
                                              str(round(sensOn / amountSens, 2)) + "  " + str(
                    round(sensOff / amountSens, 2)))
            ...
            for x in RULES:
                if (x == 1):  # StateListNeigh[iter][2]<K[iter]
                    if (int(re.search(r'\d+',StateListNeigh[iter][2:4]).group()) <= int(K[iter]) and BATTERY_STATE[iterr] > 0): #int(re.search(r'\d+', x[0:2]).group())
                        NewState.append(1) #int(re.search(r'\d+', x[0:2]).group()
                        iter += 1
                    else:
                        NewState.append(0)
                    ...
                    if (int(re.search(r'\d+',StateListNeigh[iter][0:2]).group()) <= int(K[iter]) and BATTERY_STATE[iterr] > 0):
                        NewState.append(1)
                        iter += 1
                    else:
                        NewState.append(0)
                    ...
                    if (int(re.search(r'\d+',StateListNeigh[iter][2:4]).group()) >= int(K[iter]) and BATTERY_STATE[iterr] > 0):
                        NewState.append(1)
                        iter += 1
                    else:
                        NewState.append(0)
